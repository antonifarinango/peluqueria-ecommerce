...
export const addToCart = (data, qty = 1, toast) => 
    (dispatch, getState) => {
        ...
        if (isQuantityExist) {
            ...
            toast.success(`${data?.productName} añadido al carrito`);
...
export const increaseCartQuantity = 
    ...
    (dispatch, getState) => {
        ...
        if (isQuantityExist) {
            ...
            toast.error("Se ha alcanzado el límite de cantidad");
        }
...
export const removeFromCart =  (data, toast) => (dispatch, getState) => {
    ...
    toast.success(`${data.productName} eliminado del carrito`);
    localStorage.setItem("cartItems", JSON.stringify(getState().carts.cart));
}
...
export const getAddressesById = (addressId) => async (dispatch, getState) => {
    try {
        dispatch({ type: "IS_FETCHING" });
        const { data } = await api.get(`/addresses/${addressId}`);
        //Error al abrir el pdf y volver al cart => dispatch({type: "USER_ADDRESS", payload: data});
        dispatch({ type: "IS_SUCCESS" });
        return data;
    } catch (error) {
        dispatch({ 
...
export const stripePaymentConfirmation 
    = (sendData, setErrorMesssage, setLoading, toast) => async (dispatch, getState) => {
        try {
...
export const updateOrderStatusFromDashboard =
    ...
    try {
        ...
        toast.success(data.message || "Pedido actualizado exitosamente");
...
export const updateProductFromDashboard = 
    ...
    try {
        ...
        toast.success("Actualización del producto exitosa");
        reset();
        setLoader(false);
        setOpen(false);
...
export const updateProductImageFromDashboard = 
    ...
    try {
        ...
        toast.success("Carga de imagen exitosa");
        setLoader(false);
        setOpen(false);
        await dispatch(dashboardProductsAction());
    } catch (error) {
...
export const updateCategoryDashboardAction =
  ...
  async (dispatch, getState) => {
    try {
      ...
      reset();
      toast.success("Actualización de categoría exitosa");
      setOpen(false);
      await dispatch(getAllCategoriesDashboard());
    } catch (err) {
        toast.error(
